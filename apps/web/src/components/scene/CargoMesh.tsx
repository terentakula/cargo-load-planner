...
export function CargoMesh({
  ...
  const previewPositionRef = useRef<CargoPosition | null>(null);

  const previewValidRef = useRef(true);

  const activePointerId = useRef<number | null>(null);

  const pointerCaptureTarget = useRef<PointerCaptureTarget | null>(null);
  ...
  const length = millimetersToSceneUnits(orientedSize.xMm);

  const height = millimetersToSceneUnits(orientedSize.yMm);

  const width = millimetersToSceneUnits(orientedSize.zMm);
  ...
  const handlePointerDown = (event: ThreeEvent<PointerEvent>) => {
    ...
    const [currentX, , currentZ] = getCargoScenePosition(
      cargoSpace,
      cargoTemplate,
      placedCargo,
    );
  ...
  const handlePointerMove = (event: ThreeEvent<PointerEvent>) => {
    ...
    const rawPosition = getClampedCargoPositionFromSceneCenter(
      cargoSpace,
      cargoTemplate,
      placedCargo,
  ...
  const handlePointerUp = (event: ThreeEvent<PointerEvent>) => {
    ...
    if (nextPosition && previewValidRef.current) {
  ...
  const displayedColor = invalidPreview ? "#ef4444" : cargoTemplate.color;
  ...
  return (
    ...
    >
      ...
      <meshStandardMaterial
        ...
        roughness={0.65}
        metalness={0.05}
        emissive={selected || dragging ? displayedColor : "#000000"}
        emissiveIntensity={
          invalidPreview ? 0.38 : dragging ? 0.28 : selected ? 0.18 : 0
        }
