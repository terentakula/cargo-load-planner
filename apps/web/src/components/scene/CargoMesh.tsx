...
export function CargoMesh({
  ...
  const length = millimetersToSceneUnits(cargoTemplate.lengthMm);

  const width = millimetersToSceneUnits(cargoTemplate.widthMm);

  const height = millimetersToSceneUnits(cargoTemplate.heightMm);
  ...
  const handlePointerMove = (event: ThreeEvent<PointerEvent>) => {
    ...
    const nextPosition = getClampedCargoPositionFromSceneCenter(
      cargoSpace,
      cargoTemplate,
  ...
  return (
    ...
    >
      ...
      <meshStandardMaterial
        ...
        roughness={0.65}
        metalness={0.05}
        emissive={selected || dragging ? cargoTemplate.color : "#000000"}
        emissiveIntensity={dragging ? 0.28 : selected ? 0.18 : 0}
        transparent={dragging}
        opacity={dragging ? 0.78 : 1}
      />

      <Edges
        color={dragging ? "#fef3c7" : selected ? "#ffffff" : "#111827"}
        threshold={15}
      />
    </mesh>
  );
